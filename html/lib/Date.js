...
class Date extends BaseComponent {
    ...
    renderDays() {
        ...
        //当天
        let isToday = value ? year === moment(value).get("year") && month === moment(value).get('month') : false;
    ...
    _getHeader(now) {
        ...
        let prev = this.state.stage == 3 || !this.state.prevBtn ? null : React.createElement(
            "a",
            { className: "prev", onClick: this.prev.bind(this) },
            React.createElement("i", { className: "fa fa-chevron-left" })
        );
        let next = this.state.stage == 3 || !this.state.nextBtn ? null : React.createElement(
            "a",
            { className: "next", onClick: this.next.bind(this) },
            React.createElement("i", { className: "fa fa-chevron-right" })
        );
        let month = this.state.stage > 1 ? null : React.createElement(
            "a",
            { className: "month", onClick: () => {
                    this.stageChange(2);
                } },
            now.format('MM')
        );
     ...
     */
    render() {
        let className = classnames(this.props.className, 'date-picker');
